...
在设备上分配给定 _ 形状 _ 和 _ 类型 _ 的共享数组。必须在设备上调用此函数（即，从内核或设备函数）。 _shape_ 是一个整数或整数元组，表示数组的维度，必须是一个简单的常量表达式。 _ 类型 _ 是需要存储在数组中的元素的 Numba 类型。
...
在设备上分配给定 _ 形状 _ 和 _ 类型 _ 的局部数组。 _shape_ 是一个整数或整数元组，表示数组的维度，必须是一个简单的常量表达式。 _ 类型 _ 是需要存储在数组中的元素的 Numba 类型。该数组对当前线程是私有的。返回类似于数组的对象，可以像任何标准数组一样读取和写入（例如通过索引）。
